...
func _ready():
	...
	EventBus.hud_chat_visible.connect(_on_hud_chat_visible)  # Connecté une seule fois ici (était reconnecté à chaque clic menu — bug #15)
...
func _on_menu_pressed() -> void:
	EventBus.menu_pause_ouvert.emit()  # Ouvre le menu pause (la connexion hud_chat_visible vit dans _ready)
