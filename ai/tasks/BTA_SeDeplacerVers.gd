...
# _tick() déplace le personnage vers la cible à chaque frame via le composant
func _tick(_delta: float) -> Status:
	# Récupère la position cible depuis le blackboard (Node avec global_position)
	...
	# Pas encore arrivé → utilise le composant pour naviguer vers la cible
	var comp = blackboard.get_var(comp_var)     # Récupère CompDeplacementIA depuis le blackboard
	comp.se_deplacer_vers(cible.global_position)  # Délègue au composant (NavigationAgent2D)
